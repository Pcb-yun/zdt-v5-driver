...
|------|------|------|
| X42 | Emm / X | 42mm 步进电机 |
| X42S | Emm / X | 42mm 步进电机（增强版） |
| Y42 | Emm / X | 42mm 闭环步进电机 |
...
- **Ctrl_Task** — 阻塞等待命令队列，收到命令后调用 `ZDT_V5_Process_Cmd` 执行
- **Recv_Task** — 阻塞等待接收队列，收到串口数据后调用 `ZDT_V5_Receive` 解析
- **Update_Task** — 定时循环，发送参数读取命令更新电机状态
- **App_Task** — 应用逻辑，构造命令并通过队列发送
